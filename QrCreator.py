...
class MainFrame(wx.Frame):
    ...
    def __init__(self, parent=None):
        ...
        sizer.Add(
            leftPanel, 1, wx.EXPAND | wx.LEFT | wx.TOP | wx.BOTTOM, border=20
        )  # 左侧Panel加入Sizer
        sizer.Add(line, 0, wx.EXPAND | wx.ALL, border=10)  # 分隔线加入Sizer
        sizer.Add(
            rightPanel, 1, wx.EXPAND | wx.RIGHT | wx.TOP | wx.BOTTOM, border=20
        )  # 右侧Panel加入Sizer
    ...
    def InitLeftPanel(self, panel):
        ...
        sizer = wx.GridBagSizer(
            5, 5
        )  # 参数的意义是Sizer内的控件之间横向和纵向的像素间隔

        # 二维码文本输入框
        text = wx.StaticText(panel, -1, "文本：")
        sizer.Add(text, (0, 0))
        self.tc = wx.TextCtrl(
            panel, -1, "", style=wx.TE_MULTILINE | wx.TE_PROCESS_ENTER
        )
        ...
        self.size = wx.Slider(
            panel, -1, 1, 1, 5, style=wx.SL_HORIZONTAL | wx.SL_AUTOTICKS
        )
        ...
        choices = ["低：约修正7%的错误", "中：约修正15%的错误", "高：约修正30%的错误"]
        ...
        self.logo = filebrowse.FileBrowseButton(
            panel,
            -1,
            labelText="",
            buttonText="浏览",
    ...
    def OnClipBoard(self, evt):
        ...
        if len(self.tc.GetValue()) > 0:
            if wx.TheClipboard.Open():
                ...
                wx.MessageDialog(
                    self, "成功复制到剪切板。", APP_TITLE, wx.OK | wx.ICON_INFORMATION
                ).ShowModal()
            else:
                wx.MessageDialog(
                    self, "访问剪切板失败。", APP_TITLE, wx.OK | wx.ICON_ERROR
                ).ShowModal()
    ...
    def ShowQrCode(self, evt):
        ...
        if len(text) > 0:
            ...
            # 生成二维码图片
            img = qr.make_image(
                fill_color=self.fgColor.GetValue().GetAsString(),
            ...
            if len(logoPath) > 0 and os.path.isfile(logoPath):
                ...
                icon_w = min(icon_w, size_w)
                icon_h = min(icon_h, size_h)
                # 重新设置logo的尺寸
